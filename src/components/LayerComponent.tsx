...
export const LayerComponent: React.FC<LayerProps> = ({ layer }) => {
    ...
    return (
        <div className={`${styles.layer} ${layer.type === LayerType.Input ? styles.inputLayer : ''} ${layer.type === LayerType.Output ? numberOfLayers === 2 ? styles.outputLayerWithoutHidden : styles.outputLayer : ''} ${layer.type === LayerType.Hidden ? styles.hiddenLayer : ''} `}>
            {layer.type === LayerType.Hidden && (
                <div className={styles.layerControls}>
                    <button className={styles.circleButton} onClick={handleAddNeuron}>+</button>
                    <button className={styles.circleButton} onClick={handleDeleteNeuron}>-</button>
                </div>
            )}
